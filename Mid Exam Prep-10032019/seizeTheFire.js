...
If you don’t have enough water to put out a given cell – skip it and try the next one. In the end, print the cells 
...
Input / Constraints
•	On the 1st line you are going to receive the fires with their cells in the format described above – integer numbers 
in the range [1…500]
...
function seizeTheFire(input) {
    ...
    for (let i = 0; i < fires.length; i++) {
        ...
        let current = fires[i].split(' = ');
        let command = current[0];
        let strength = Number(current[1]);
        ...
        if (strength >= minRange && strength <= maxRange && water >= strength) {
            water -= strength;
            totalFire += strength;
            effort += strength * 0.25;
            finishedFires.push(strength);

        }
    }
    console.log('Cells:');
    for (let element of finishedFires) {
        console.log(`- ${element}`);
